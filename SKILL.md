...
- For revoked/cancelled/surrendered/withdrawn/expired licenses, leave score and `ai` blank by default. Do not ask Gemini for an explanation and do not write AI Introduction for inactive licenses unless the user explicitly requests it for a specific record.
...
6. Stage 1 — run the official-website precheck from `official-website-precheck.md`. Compare website-disclosed regulators, legal entities, license numbers, and scopes with IFXData Global license records before scoring:
   ...
   - If IFXData contains a license that the official website no longer discloses, run the two-condition expired-license test. Only when website absence and Gemini cancellation confirmation both pass, update that license `status` to `Revoked` through `updateLicense`, verify by fresh API read, and leave score/`ai` blank. If the test does not fully pass, leave the license unchanged and continue to the next eligible license.
...
7a. For records whose current `status` is already `Revoked`, `Cancelled`, `Surrendered`, `Withdrawn`, or `Expired`, skip scoring and leave score/`ai` blank unless the user explicitly requests an inactive-license explanation for that specific record.
