...
- If the official website or regulator discloses missing licenses, the user has authorized automatic missing-license additions for this workflow. Add clear missing license records directly through the IFXData API instead of stopping for confirmation. Add only records supported by official broker/regulator evidence, use existing IFXData license-type options or a verified newly-created standard type, split multi-activity SFC records into one license row per activity type, and verify every added record by API read before Gemini scoring. Stop only when required fields are unclear, the closest license type is genuinely uncertain, the official source is contradictory, or API read-after-write verification fails.
- If IFXData has a license that the broker's official website no longer discloses, do not mark it revoked from website absence alone. Run a focused Gemini cancellation check using the broker name and the exact license fields. Change IFXData `status` to `Revoked` only when both conditions are true: (1) the official website cannot be found to disclose that license, and (2) Gemini clearly confirms the license was revoked/cancelled/withdrawn/surrendered or voluntarily cancelled, with a specific reason or evidence context. If either condition is missing or uncertain, keep the existing backend record unchanged and continue the automation.
- For revoked/cancelled/surrendered/withdrawn/expired licenses, do not save a score by default, but do ask Gemini for the inactive-license reason/background and write that explanation to `ai`. The explanation should cover whether the license was revoked, cancelled, withdrawn, surrendered, expired, or voluntarily cancelled; the likely reason/timing context; whether the broker still discloses it; and investor significance. Keep `score` blank unless the user explicitly requests a score for inactive licenses.
...
- Use DeepSeek as the default supporting layer for non-scoring language/structure tasks when `DEEPSEEK_API_KEY` is available: official-disclosure structuring, website-vs-backend difference summaries, license-type suggestions, missing-field candidate extraction, English Gemini-introduction compression, license-type translation/description drafting, Chinese batch reports, API-log summaries, and exception review. Do not use DeepSeek for final license scoring, API write decisions, revoked/cancelled confirmation, read-after-write verification, or regulator fact verification.
- DeepSeek routing is allowed only for auxiliary work that does not decide the final score or write safety. Use DeepSeek for: structuring public official website/regulator disclosure into candidate license rows; summarizing website-vs-backend differences; suggesting the closest existing IFXData license type from a supplied dropdown list; extracting candidate values for empty `country`, `beginTime`, `email`, `telphone`, `address`, and `fullName`; compressing long Gemini AI Introductions without changing facts; translating/reporting Chinese summaries; summarizing API logs; drafting English license-type range/note text; and reviewing redacted exceptions. Do not use DeepSeek for: final license score, revoked/cancellation confirmation, regulator fact verification, deciding whether to write/update/add, or replacing fresh API read verification.
...
6. Stage 1 — run the official-website precheck from `official-website-precheck.md`. Compare website-disclosed regulators, legal entities, license numbers, and scopes with IFXData Global license records before scoring:
   ...
   - Use DeepSeek by default to structure public disclosure text (`deepseek_license_pipeline.py disclosure`) and, when backend comparison is needed, summarize official-website-vs-IFXData differences (`deepseek_license_pipeline.py compare`). Final match/write decisions must still follow local rules plus official/IFXData evidence.
   - If the official website shows mismatched company names, license numbers, scopes, or extra backend licenses, use DeepSeek to structure/summarize the public disclosure or current-broker difference list when available, then stop before Gemini scoring and report the differences so the user can correct IFXData first.
...
7. Run local source checks before querying Gemini. Confirm institution, license type, license number, status, and company registration name are present. Apply known local special rules:
   ...
   - When optional fields are empty, use DeepSeek for candidate extraction only when it can reduce Gemini prompt size or organize public evidence (`deepseek_license_pipeline.py missing-fields`). Gemini must still be asked for supplemental fields during scoring, and only concrete values may be written into empty IFXData fields.
7a. For records whose current `status` is already `Revoked`, `Cancelled`, `Surrendered`, `Withdrawn`, or `Expired`, skip normal scoring but ask Gemini for an inactive-license explanation and write it to `ai` when `ai` is empty or stale. Keep `score` blank unless the user explicitly requests a score for inactive licenses.
...
10a. If Gemini returns a useful but overly long `ai` introduction, compress it with DeepSeek (`deepseek_license_pipeline.py compress --max-words 400`) before staging, preserving all substantive reasoning, risk language, score context, and uncertainty. Do not compress if the Gemini text is already concise enough for IFXData.
...
14. At broker or batch end, summarize local result rows and API logs. Include the official-website precheck status, completed/skipped/needs-review records, and next broker. Use DeepSeek (`report` or `log-summary`) for the Chinese summary when configured; if unavailable, produce the summary locally.
...
| Website-vs-backend difference summary | Yes | Use `compare`; send only current-broker fields needed for exact comparison and never credentials/cookies/screenshots. |
| License type suggestion | Yes | Provide current dropdown options; local standardization rules decide final type. |
| Missing field candidate extraction | Yes | Use `missing-fields`; candidate values only, write only when empty and concrete. |
...
| API log summary | Yes | Use `log-summary`; do not expose raw secrets or recommend duplicate writes. |
| License type description/range translation | Yes | Draft/translate concise English `note` and `typeRange`; user/local rules approve creation. |
