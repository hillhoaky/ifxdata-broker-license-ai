...
- Do not leave revoked/cancelled/surrendered/withdrawn/expired licenses without an AI explanation. For a license already marked inactive, or a license newly changed to `Revoked` after the two-condition test, ask Gemini for the revocation/cancellation reason and background using the exact license fields and available evidence. Write Gemini's explanatory English text into `ai`. Use a low risk score only; if Gemini returns a score above 20 for an inactive license, cap the saved score at 20 unless the user explicitly authorizes a different scoring policy.
...
7a. For records whose current `status` is already `Revoked`, `Cancelled`, `Surrendered`, `Withdrawn`, or `Expired`, do not skip the record silently. Ask Gemini for an inactive-license explanation focused on why/when/how the license became inactive, investor significance, and whether the broker still discloses it. Write the explanation to `ai`, save a low score capped at 20, verify by fresh API read, and continue.
