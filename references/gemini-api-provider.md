...
3. Target no more than 1 Gemini request every 20 seconds when using the configured API / relay provider such as Google AI Studio or QuickRouter.
4. Wait at least 20 seconds between license-scoring calls. Pass `--min-interval 20` or set `GEMINI_MIN_INTERVAL_SECONDS=20`; the script uses a shared local rate-limit file so parallel Codex sessions on the same machine queue instead of colliding.
5. After each broker, pause at least 60 seconds before starting the next broker in the same batch.
5a. If a broker has 7 or more eligible licenses, keep the 60-second cool-down after completing, rate-limiting, or otherwise stopping that broker before starting any new broker or unresolved-license retry.
...
- `GEMINI_MIN_INTERVAL_SECONDS=20`
...
When CodexA and CodexB run on different machines or environments, the local shared file cannot coordinate both workers. In that case, keep the same 20-second per-request pace on each worker, pause 60 seconds between brokers, and consider separate provider keys/projects or an IFXData-owned central queue for production batches.
