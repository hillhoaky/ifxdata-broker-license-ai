...
- If the website discloses licenses missing from IFXData, do not ask Gemini to score stale backend-only data yet. Build an add-license candidate from concrete fields found from the official website/regulator:
...
- If IFXData contains a license the website no longer discloses, mark it `website_missing` and run the two-condition expired-license test:
  ...
  If both conditions are satisfied, update IFXData `status` to `Revoked` through `updateLicense`, verify by fresh API read, then ask Gemini for the inactive-license reason/background and write the explanation to `ai` while keeping `score` blank. If either condition is missing, ambiguous, or contradicted, leave the license unchanged and continue the automation.

## Inactive license explanation

For records already marked `Revoked`, `Cancelled`, `Surrendered`, `Withdrawn`, or `Expired`, skip normal scoring but ask Gemini for an inactive-license explanation and write it to `ai` when `ai` is empty or stale. Keep `score` blank unless the user explicitly requests a score for inactive licenses. Do not use DeepSeek for cancellation confirmation or inactive-license facts; DeepSeek may only summarize the final redacted result for reporting.
