...
- For `type` corrections, choose the closest existing IFXData `Type of license` value instead of creating free-form wording. If the closest type is uncertain, show the nearest option to the user and ask for confirmation. If no close option exists, ask the user to manually create a new type before continuing. Current mappings: FinCEN/MSB registration -> `Money Services Business`; crypto exchange / virtual asset registration -> `Virtual asset service provider`.
