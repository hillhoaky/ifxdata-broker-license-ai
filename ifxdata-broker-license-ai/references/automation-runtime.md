...
- Gemini API scoring must be sequential by default: one broker at a time per worker, no parallel license scoring, target no more than 1 request every 120 seconds when CodexA/CodexB share the same Gemini key/project, pause at least 600 seconds between brokers, keep a daily safety cap of about 1,000 Gemini scoring calls per API key unless a higher active AI Studio limit is confirmed, retry HTTP 429 with 180-second then about 300-second backoff, then stop with `gemini_rate_limited`.
